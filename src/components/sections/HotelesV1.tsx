        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
          {hotels.map((hotel, idx) => (
            <Reveal key={idx} delay={idx * 0.06}>
              <div className="group h-full flex flex-col border border-wine/10 rounded-xl p-5 bg-white hover:border-wine/30 hover:shadow-md transition-all duration-300">
                {/* Iniciales */}
                <div className="w-10 h-10 rounded-lg bg-wine/8 flex items-center justify-center mb-4 flex-shrink-0">
                  <span className="font-serif text-sm font-bold text-wine/70 tracking-wide">
                    {hotel.initials}
                  </span>
                </div>

                {/* Nombre */}
                <h3 className="font-serif text-base md:text-lg text-wine leading-snug mb-3">
                  {hotel.name}
                </h3>
                ...
                {/* Nota */}
                {hotel.note && (
                  <p className="text-xs text-wine/50 mt-1 leading-snug">
                    {hotel.note[lang]}
                  </p>
                )}
                ...
                {hotel.href ? (
                  <a
                    ...
                    className="mt-4 inline-block text-[10px] font-bold uppercase tracking-widest text-wine border border-wine/30 rounded-full px-4 py-2 text-center hover:bg-wine hover:text-paper transition-all duration-200"
                  >
                    {t("hotelesReservar", language)}
                  </a>
                ) : (
                  <p className="mt-4 text-[10px] uppercase tracking-widest text-wine/40 font-bold">
                    {t("hotelesConsultar", language)}
                  </p>
                )}
              </div>
            </Reveal>
