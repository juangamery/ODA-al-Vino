  ...
  if (confirmation) {
    return (
      <div className="mx-auto max-w-lg px-5 py-24 text-center">
        ...
        <ul className="mb-8 space-y-2 rounded-2xl border border-wine/15 bg-white/50 p-5 text-left text-sm text-wine/90">
  ...
  return (
    <div className="min-h-screen bg-paper pb-28 md:pb-16">
      <header className="px-5 pb-4 pt-10 text-center md:pt-16">
        ...
        <h1 className="mt-2 text-2xl text-wine md:text-4xl">Inscripción a salas de degustación</h1>
        <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-wine/70 normal-case tracking-normal font-sans">
      ...
      <div className="mx-auto grid max-w-[1180px] grid-cols-1 gap-6 px-5 pb-10 md:grid-cols-[1fr_340px] md:items-start">
            ...
            return (
              <div key={slot} className="mb-8">
                <div className="mb-3 flex items-center gap-3">
                  <span className="lato-expanded text-[11px] text-plum">{slot} hs</span>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {salaIds.map((salaId) => {
                    ...
                    return (
                      ...
                      >
                        <div className="lato-expanded text-[10px] text-plum">{sala.nombre}</div>
                        <div className="mt-1 font-serif text-lg normal-case tracking-normal text-wine">
                          {cata.bodega}
                        </div>
                        <div className={`mt-1 text-[13px] ${cata.profesional ? "text-wine/70" : "italic text-wine/40"}`}>
                        ...
                        <div className="mt-3 flex items-center justify-between">
                          ...
                          {hasExtra && (
                            <button
                              ...
                              className="text-[11.5px] text-plum underline"
                            >
                              Ver más
                            </button>
                          )}
                        ...
                        {hasExtra && detailsOpen && (
                          <div className="mt-3 space-y-1 border-t border-dashed border-wine/20 pt-3 text-[12.5px] text-wine/70">
                            {cata.restaurante && (
                              <div>
        ...
        >
          ...
          >
            <span className="font-serif text-base normal-case tracking-normal text-wine">Tu ticket · {totalSelected}</span>
            <span className={`transition-transform ${ticketExpanded ? "rotate-180" : ""}`}>▲</span>
          </button>

          <h3 className="hidden font-serif text-lg normal-case tracking-normal text-wine md:block">Tu ticket</h3>
          <p className="mb-3 mt-1 hidden text-xs text-wine/60 md:block">
          ...
          <div className="mt-3 md:mt-0">
            {DAYS.map((d) => (
              <div key={d.id} className="mb-3">
                <div className="lato-expanded mb-1.5 text-[10px] text-plum">{d.label}</div>
                {selections[d.id].length === 0 ? (
                  <p className="text-[12.5px] italic text-wine/40">Todavía no elegiste ninguna sala.</p>
                ) : (
                  selections[d.id].map((sel) => {
                    ...
                    return (
                      <div
                        ...
                        className="mb-1.5 flex items-start justify-between gap-2 rounded-lg bg-wine/5 px-2.5 py-2 text-[12.5px]"
                      >
                        <div>
                          <b className="block text-[13.5px] text-wine">{cata.bodega}</b>
                          <span className="text-[11.5px] text-wine/60">
                            {sala.nombre} · {sel.slot}
                          </span>
                        </div>
                        <button
                          onClick={() => toggleSelection(d.id, sel.slot, sel.salaId)}
                          className="text-base leading-none text-wine/50 hover:text-plum"
                        >
                          ✕
                        </button>
                      </div>
            ...
            <div className="mb-3">
              <label htmlFor="nombre" className="lato-expanded mb-1 block text-[10px] text-plum">
                Nombre y apellido
              </label>
              <input
                id="nombre"
                ...
                className="w-full rounded-lg border border-wine/25 bg-white px-3 py-2.5 text-sm text-wine placeholder:text-wine/35 focus:outline-none focus:ring-2 focus:ring-harvest"
              />
            </div>
            <div className="mb-3">
              <label htmlFor="contacto" className="lato-expanded mb-1 block text-[10px] text-plum">
                Email
              </label>
              <input
                id="contacto"
              ...
              {participantStatus === "checking" && (
                <p className="mt-1.5 text-[11.5px] text-wine/50">Verificando participante…</p>
              )}
              {participantStatus === "confirmed" && (
                <p className="mt-1.5 text-[11.5px] text-harvest">✓ Participante confirmado de ODA al Vino 2026</p>
              )}
              {participantStatus === "not_found" && (
                <p className="mt-1.5 text-[11.5px] text-plum">
            ...
            {formError && (
              <div className="mt-2.5 rounded-lg border border-plum/40 bg-plum/10 px-3 py-2 text-[13px] text-plum">
                {formError}
              </div>
            )}
          </div>
